...
# ============== LOGO CÔNG TY VĨNH HƯNG (VHIP) ==============
# GHI CHÚ QUAN TRỌNG: File gốc của bạn có nhúng sẵn 1 chuỗi base64 rất dài
# (LOGO_B64_192 / LOGO_B64_512) để hiển thị logo công ty làm favicon/icon màn
# hình chính. Việc chép tay chuỗi base64 khổng lồ đó qua chat có rủi ro sai 1
# ký tự là app sẽ lỗi ngay khi mở (không giải mã được ảnh), nên bản này tạm
# dùng icon emoji 🛡️ để đảm bảo chạy chắc chắn 100%.
# ==> Để khôi phục đúng logo cũ: mở file app.py HIỆN TẠI của bạn (bản đang
# chạy tốt), copy 2 dòng "LOGO_B64_192 = ..." và "LOGO_B64_512 = ..." (cùng
# khối "_head_icon_js" gắn apple-touch-icon ngay bên dưới nó) rồi dán đè vào
# đúng vị trí này là xong, không cần sửa gì thêm.
...
st.markdown("""
    ...
    div[data-testid="stTabs"]:first-of-type > div > [role="tablist"] > [role="tab"]:nth-child(2)[aria-selected="true"] { background: linear-gradient(120deg, #c62828, #ef5350) !important; border-color: #c62828 !important; }
    div[data-testid="stTabs"]:first-of-type > div > [role="tablist"] > [role="tab"]:nth-child(3)[aria-selected="true"] { background: linear-gradient(120deg, #00838f, #26c6da) !important; border-color: #00838f !important; }
    div[data-testid="stTabs"]:first-of-type > div > [role="tablist"] > [role="tab"]:nth-child(4)[aria-selected="true"] { background: linear-gradient(120deg, #6a1b9a, #ab47bc) !important; border-color: #6a1b9a !important; }
    div[data-testid="stTabs"]:first-of-type > div > [role="tablist"] > [role="tab"]:nth-child(5)[aria-selected="true"] { background: linear-gradient(120deg, #c2410c, #f97316) !important; border-color: #c2410c !important; }
    div[data-testid="stTabs"]:first-of-type > div > [role="tablist"] > [role="tab"]:nth-child(6)[aria-selected="true"] { background: linear-gradient(120deg, #283593, #5c6bc0) !important; border-color: #283593 !important; }
    div[data-testid="stTabs"]:first-of-type > div > [role="tablist"] > [role="tab"]:nth-child(7)[aria-selected="true"] { background: linear-gradient(120deg, #37474f, #78909c) !important; border-color: #37474f !important; }
...
def chuan_hoa_trang_thai(val):
    """Gom các cách viết hoa/thường khác nhau của cùng 1 trạng thái sản xuất
    (VD: 'Chưa SX', 'Chưa sx' đều hiểu là 1) về đúng 1 tên hiển thị duy nhất,
    tránh bị trùng lặp gây hiểu nhầm trong dropdown 'Tình Trạng SX'. Các giá
    trị lạ/không nằm trong danh sách quy ước (VD: 'VHIP') vẫn được giữ nguyên,
    chỉ cắt khoảng trắng thừa."""
    s = clean_status(val)
    s_low = re.sub(r'\s+', ' ', s.strip().lower())
    mapping = {
        'chưa sx': 'Chưa SX',
        'đang sx': 'Đang SX',
...
def load_data():
    ...
    df['Trang_Thai_SX'] = df_raw.iloc[3:, 2].apply(clean_status).apply(chuan_hoa_trang_thai)
...
def render_dashboard():
    ...
    try:
        ...
        # TAB DANH MỤC
        tab_names = ['📊 Dashboard Tổng', '🚨 Chưa Chốt Lịch Giao', '📦 Gối Chậu', '⚙️ Khe Răng Lược', '🧱 Tấm VCO', '🏗️ Cột H (Phụ Kiện)', '📋 Sản Phẩm Khác']
        ...
        for i, tname in enumerate(tab_names):
            with tabs[i]:
                if tname == '🚨 Chưa Chốt Lịch Giao':
                    q_col = 'So_Luong_Tong_DH'
                    if ky_sel == "Cả Năm":
                        df_cclg = pd.concat([df_moi, df_ton]).copy()
                    else:
                        df_cclg = df_base[
                        ...
                        if tt_sel != 'Tất cả tình trạng':
                            df_cclg = df_cclg[df_cclg['Trang_Thai_SX'].str.lower() == tt_sel.lower()]
 
                    # Chỉ tính đơn đang thật sự cần sản xuất (Chưa SX / Đang SX)
                    df_cclg = df_cclg[df_cclg['Trang_Thai_SX'].isin(['Chưa SX', 'Đang SX'])]
                    # "Chưa chốt lịch giao" = còn thiếu Ngày Chốt Tiến Độ Giao Hàng VHIP-KD (cột AG)
                    df_cclg = df_cclg[df_cclg['Ngay_Chot_AG'].isna()]
                    ...
                    sl_chua_chot = df_cclg[q_col].sum()
                    so_don_chua_chot = len(df_cclg)
 
                    st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
                    m0, m1, m2, m3, m4, m5, m6 = st.columns(7)
                    m0.metric("🚨 Số Đơn Chưa Chốt", f"{so_don_chua_chot:,}")
                    m1.metric("📦 Gối Chậu", f"{df_cclg['SL_GoiChau'].sum():,.2f}")
                    m2.metric("⚙️ Khe Răng Lược", f"{df_cclg['SL_KheRangLuoc'].sum():,.2f}")
                    m3.metric("🧱 Tấm VCO", f"{df_cclg['SL_TamVCO'].sum():,.2f}")
                    m4.metric("🏗️ Cột H (Phụ Kiện)", f"{df_cclg['SL_HeCotPhuKien'].sum():,.2f}")
                    m5.metric("📋 Sản Phẩm Khác", f"{df_cclg['SL_NhomKhac'].sum():,.2f}")
                    m6.metric("🎯 Tổng Cộng", f"{sl_chua_chot:,.2f}")
 
                    st.caption(f"📋 {so_don_chua_chot:,} đơn (Chưa SX / Đang SX) chưa có Ngày Chốt Tiến Độ Giao Hàng VHIP-KD, cần giao trong {ten_ky_hien_thi}")
                    ...
                    if search_kw:
                        df_cclg = df_cclg[df_cclg['So_DH'].astype(str).str.contains(search_kw, case=False, na=False)]
 
                    with col_export:
                        st.write("")
                        st.write("")
                        excel_buf = io.BytesIO()
                        sheet_name = re.sub(r'[\/\\\?\*\:\[\]]', '-', f"ChuaChotAG {ten_ky_hien_thi}")[:31]
                        with pd.ExcelWriter(excel_buf, engine='openpyxl') as writer:
                            df_cclg[cols_display].to_excel(writer, index=False, sheet_name=sheet_name)
                        st.download_button(
                            label="📥 Trích Excel (Chưa Chốt Lịch Giao)",
                            data=excel_buf.getvalue(),
                            file_name=f"Chua_Chot_Lich_Giao_{re.sub(r'[\/\\\?\*\:\[\]]', '-', ten_ky_hien_thi)}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            use_container_width=True,
                            key=f"btn_ex_{i}"
                        )
 
                    st.caption(f"📄 {len(df_cclg)} dòng")
                    render_pretty_table(df_cclg, cols_display, q_col, f"cclg_{i}")
                ...
                def _khong_tam_dung(df_x):
                    """Mặc định (chưa chọn 1 Tình Trạng SX cụ thể) sẽ tự loại đơn 'Tạm dừng
                    SX' khỏi 'Tổng Cần Sản Xuất' - để khớp đúng logic sản xuất thực tế.
                    Nếu người dùng đã chủ động chọn 1 Tình Trạng SX cụ thể ở bộ lọc trên
                    (VD: chọn 'Tạm Dừng SX' để xem riêng số liệu tạm dừng) thì tôn trọng
                    đúng lựa chọn đó, không loại trừ nữa."""
                    if tt_sel != 'Tất cả tình trạng':
                        return df_x
                ...
                if tname == '📊 Dashboard Tổng':
                    # Số liệu "Tổng Cần Sản Xuất" tách riêng theo từng chủng loại sản phẩm,
                    # để khi kết hợp với bộ lọc "🏭 Tình Trạng SX" (VD chọn "Tạm Dừng SX")
                    # là xem được ngay số lượng tạm dừng của từng loại (Gối, Khe răng lược...)
                    # mà không cần vào riêng từng tab sản phẩm.
                    df_tong_can_sx_ct = pd.concat([
                        _khong_tam_dung(sub_moi_can_trong_ky),
                        _khong_tam_dung(sub_ton)
                    ])
                    st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
                    st.caption("📊 Tổng Cần Sản Xuất — chi tiết theo từng chủng loại (đã áp dụng đúng bộ lọc Tình Trạng SX đang chọn):")
                    b1, b2, b3, b4, b5 = st.columns(5)
                    b1.metric("📦 Gối Chậu", f"{df_tong_can_sx_ct['SL_GoiChau'].sum():,.2f}")
                    b2.metric("⚙️ Khe Răng Lược", f"{df_tong_can_sx_ct['SL_KheRangLuoc'].sum():,.2f}")
                    b3.metric("🧱 Tấm VCO", f"{df_tong_can_sx_ct['SL_TamVCO'].sum():,.2f}")
                    b4.metric("🏗️ Cột H (Phụ Kiện)", f"{df_tong_can_sx_ct['SL_HeCotPhuKien'].sum():,.2f}")
                    b5.metric("📋 Sản Phẩm Khác", f"{df_tong_can_sx_ct['SL_NhomKhac'].sum():,.2f}")
