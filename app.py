...
# 2. BỘ MÃ CSS + JS ĐIỀU CHỈNH GIAO DIỆN & TIÊU ĐỀ TAB (ÉP MÀU TỐI ĐA)
st.markdown("""
    <style>
    /* Bố cục chung */
    .main { padding: 1rem; }
    ...
    /* ========================================================= */
    /* CSS ÉP TẤT CẢ CÁC KIỂU PHIÊN BẢN PHỦ RỘNG BẢNG TAB        */
    /* ========================================================= */
    
    /* Vùng chứa danh sách Tab */
    ...
    /* CÁC NÚT TAB THƯỜNG (CHƯA CHỌN) */
    div[data-testid="stTabs"] [role="tab"],
    div[data-baseweb="tab-list"] button,
    .stTabs [role="tab"] {
        background-color: #f1f5f9 !important;
        border: 2px solid #64748b !important; /* Viền xám đậm nổi bật */
        border-radius: 8px !important;
        padding: 8px 18px !important;
        transition: all 0.2s ease-in-out !important;
    }

    /* Chữ bên trong các Nút Tab Thường */
    div[data-testid="stTabs"] [role="tab"] *,
    div[data-baseweb="tab-list"] button *,
    .stTabs [role="tab"] * {
        color: #0f172a !important; /* Chữ đen đậm nét */
    ...
    .stTabs [role="tab"][aria-selected="true"] {
        background-color: #1d4ed8 !important; /* Nền màu xanh dương nổi bật */
        border: 2px solid #1e40af !important;
        box-shadow: 0 4px 10px rgba(29, 78, 216, 0.4) !important;
    }

    /* Chữ bên trong Tab Đang Được Chọn */
    div[data-testid="stTabs"] [role="tab"][aria-selected="true"] *,
    div[data-baseweb="tab-list"] button[aria-selected="true"] *,
    .stTabs [role="tab"][aria-selected="true"] * {
    ...
    <!-- SCRIPT CAN THIỆP TRỰC TIẾP VÀO DOM TRÌNH DUYỆT -->
    <script>
    function styleStreamlitTabs() {
        const doc = window.parent.document || document;
        const tabs = doc.querySelectorAll('div[data-testid="stTabs"] button[role="tab"], div[data-baseweb="tab-list"] button');
        tabs.forEach(tab => {
            const isSelected = tab.getAttribute('aria-selected') === 'true';
            if (isSelected) {
                tab.style.setProperty('background-color', '#1d4ed8', 'important');
                tab.style.setProperty('border', '2px solid #1e40af', 'important');
                const children = tab.querySelectorAll('*');
                children.forEach(c => c.style.setProperty('color', '#ffffff', 'important'));
            } else {
                tab.style.setProperty('background-color', '#f1f5f9', 'important');
                tab.style.setProperty('border', '2px solid #64748b', 'important');
                const children = tab.querySelectorAll('*');
                children.forEach(c => c.style.setProperty('color', '#0f172a', 'important'));
            }
        });
    }
    
    // Tự động quét và áp dụng giao diện mỗi 300ms
    setInterval(styleStreamlitTabs, 300);
    </script>
...
def render_dashboard():
    ...
    try:
        ...
        # TAB DANH MỤC CÓ KHUNG BAO VÀ CHỮ IN ĐẬM
