...
def clean_status(val):
    ...
    ban_do_chuan_hoa = {
        ...
        'tam dung san xuat': 'Tạm Dừng SX',
        'tạm dừng sản xuất': 'Tạm Dừng SX',
        'done': 'Done',
        'hoan thanh': 'Done',
        'hoàn thành': 'Done',
    }
        ...
        for i, tname in enumerate(tab_names):
            with tabs[i]:
                ...
                if tt_sel != 'Tất cả tình trạng':
                    ...
                    sl_moi_trong_tt = sub_moi[q_col].sum()
                    sl_luy_ke_tamdung = sub_ton[q_col].sum()
                    sl_tong_trang_thai = sl_moi_trong_tt + sl_luy_ke_tamdung
                    sl_da_nhap_kho_tt = sub_done[q_col].sum()
                    sl_con_lai_tt = max(0, sl_tong_trang_thai - sl_da_nhap_kho_tt)
