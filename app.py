...
# Hàm xử lý chuyển đổi số chính xác tuyệt đối
...
def load_data():
    ...
    df = pd.DataFrame({
        ...
        'Nam_Dat_Hang_Raw': df_raw.iloc[4:, 3],       # Cột D: Năm
        'Bo_Phan_KD': df_raw.iloc[4:, 4],             # Cột E
        'NV_KD': df_raw.iloc[4:, 6],                 # Cột G
        'Du_An': df_raw.iloc[4:, 7],                 # Cột H
        'Quy_Cach': df_raw.iloc[4:, 9],              # Cột J: Quy cách
        'DVT': df_raw.iloc[4:, 10],                  # Cột K
        'Nhom_SP': df_raw.iloc[4:, 12],              # Cột M: Nhóm SP
    ...
    # Bổ sung thông tin cho các dòng bị Merge
    df['So_DH'] = df['So_DH'].replace('', None).ffill()
    df['Nam_Col_D'] = df['Nam_Dat_Hang_Raw'].replace('', None).ffill().astype(str).str.extract(r'(\d{4})')[0]
    
    # Loại bỏ các dòng tiêu đề tổng
    df = df[df['So_DH'].notna()]
    df = df[~df['So_DH'].astype(str).str.contains('Tổng|Tong|TỔNG|STT|Số ĐH', case=False, na=False)]
    
    # Ép kiểu số lượng
    df['So_Luong_Tong_DH'] = df['So_Luong_Tong_DH_Raw'].apply(parse_quantity)
    
    # Trích xuất năm chuẩn
...
try:
    ...
    # Từ khóa mở rộng để bao phủ toàn bộ các tên/chủng loại Gối & Chậu
    kw_goi_chau = 'Gối|Chậu|gối|chậu|pot|Pot|Chau|Goi|chỏm|chom|cao su|thép|chậu cao su|gối cao su|gối thép|chậu thép'
    ...
    for i, tab_name in enumerate(nhom_sp_list):
        with tabs[i]:
            if tab_name == '📊 Dashboard Tổng':
                ...
                kw_ex = 'Gối|Chậu|gối|chậu|Khe|Lược|khe|lược|Tấm|VCO|tấm|vco|Cột|Phụ Kiện|cột|pot|Pot|chỏm|chom'
                df_tab = df_filtered[~df_filtered['Nhom_SP_Clean'].str.contains(kw_ex, case=False, na=False)]

            # 5. HIỂN THỊ THỐNG KÊ METRIC
