...
with col_btn:
    st.write("") # Tạo khoảng trống căn chỉnh
...
# Đã giảm ttl xuống 5 giây để giảm độ trễ khi cập nhật
@st.cache_data(ttl=5)
def load_data():
    ...
    df['Ngay_KD_Gui_DH_DT'] = parse_date_robust(df_raw.iloc[4:, idx_ngay_kd_gui], 'Ngày KD gửi ĐH')
    df['Ngay_Duyet_DH_DT'] = parse_date_robust(df_raw.iloc[4:, idx_ngay_duyet], 'Ngày duyệt hoàn toàn')
    df['Ngay_YCGH_DT'] = parse_date_robust(df_raw.iloc[4:, idx_ngay_ycgh], 'Ngày YCGH')
    df['Ngay_Chot_Cuoi_DT'] = parse_date_robust(df_raw.iloc[4:, idx_ngay_chot], 'Ngày chốt cuối')
    df['Ngay_Nhap_Kho_DT'] = parse_date_robust(df_raw.iloc[4:, idx_ngay_nhapkho], 'Ngày thực tế nhập kho')
...
try:
    ...
    with st.expander("🔧 Thông tin gỡ lỗi (Debug) — bấm để xem"):
        st.caption(f"Tổng số cột đọc được: {debug_info['so_cot_doc_duoc']} | Tổng số dòng: {debug_info['so_dong_doc_duoc']}")
    ...
    for i, tab_name in enumerate(nhom_sp_list):
        with tabs[i]:
            if tab_name == '📊 Dashboard Tổng':
                ...
                df_tab_nhap = df_nhap.copy()
                total_so_luong = df_tab['So_Luong_Tong_DH'].sum()
                total_nhap_kho = df_tab_nhap.loc[df_tab_nhap['Da_Nhap_Kho'], 'So_Luong_Tong_DH'].sum()
            ...
            m2.metric("📦 Số lượng tổng ĐH", f"{total_so_luong:,.2f}")
            m3.metric("✅ Tổng SL Nhập Kho", f"{total_nhap_kho:,.2f}")
            m4.metric("⏳ SL Tồn Cần Sản Xuất", f"{total_ton_kho:,.2f}")

            st.markdown("<br>", unsafe_allow_html=True)

            search_kw = st.text_input(f"🔍 Tìm kiếm trong tab [{tab_name}]:", key=f"search_input_{i}")
