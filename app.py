...
try:
    ...
    # --- SỬA LỖI: chỉ dùng từ khóa CÓ DẤU, khớp trên cột Nhóm SP (cột M) là chính ---
    # Lý do lỗi trước đây: khi bỏ dấu, "Chau" (từ "Chậu") trùng với "Chau" (từ "Châu",
    # ví dụ "Châu Âu", "Châu Á" trong Quy Cách/xuất xứ) => match nhầm gần như toàn bộ dữ liệu.
    kw_goi_chau = 'Gối|Chậu'
    kw_khe_luoc = 'Khe|Lược'
    kw_tam_vco = 'Tấm|VCO'
    kw_cot_pk = 'Cột|Phụ Kiện'
    kw_loai_tru = 'Gối|Chậu|Khe|Lược|Tấm|VCO|Cột|Phụ Kiện'
    ...
    for i, tab_name in enumerate(nhom_sp_list):
        with tabs[i]:
            if tab_name == '📊 Dashboard Tổng':
                ...
                # Chỉ lọc theo cột Nhóm SP (cột M) - cột phân loại chuẩn
